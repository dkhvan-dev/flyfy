package config

import (
	"context"
	"testing"
	"time"
)

func TestLoadReadsCityScopedTransitFlags(t *testing.T) {
	t.Setenv("ROUTING_TRANSIT_ENABLED", "true")
	t.Setenv("ROUTING_TRANSIT_CITY_CODE", "ALA")
	t.Setenv("ROUTING_TRANSIT_GTFS_VERSION", "almaty-gtfs-2026-06-21")
	t.Setenv("ROUTING_ENABLE_OTP", "true")
	t.Setenv("OTP_URL", "http://otp:8080")

	cfg, err := Load(context.Background())
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}

	if !cfg.Data.TransitEnabled {
		t.Fatal("TransitEnabled = false, want true")
	}
	if cfg.Data.TransitCityCode != "ALA" {
		t.Fatalf("TransitCityCode = %q, want ALA", cfg.Data.TransitCityCode)
	}
	if cfg.Data.GTFSVersion != "almaty-gtfs-2026-06-21" {
		t.Fatalf("GTFSVersion = %q", cfg.Data.GTFSVersion)
	}
	if !cfg.TransitRuntimeEnabled() {
		t.Fatal("TransitRuntimeEnabled() = false, want true")
	}
}

func TestTransitRuntimeRequiresOTPURL(t *testing.T) {
	t.Setenv("ROUTING_TRANSIT_ENABLED", "true")
	t.Setenv("ROUTING_TRANSIT_CITY_CODE", "ALA")
	t.Setenv("ROUTING_TRANSIT_GTFS_VERSION", "almaty-gtfs-2026-06-21")
	t.Setenv("ROUTING_ENABLE_OTP", "true")

	cfg, err := Load(context.Background())
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}

	if cfg.TransitRuntimeEnabled() {
		t.Fatal("TransitRuntimeEnabled() = true without OTP_URL")
	}
}

func TestLoadKeepsLegacyGTFSVersionFallback(t *testing.T) {
	t.Setenv("ROUTING_GTFS_VERSION", "legacy-gtfs-version")

	cfg, err := Load(context.Background())
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}

	if cfg.Data.GTFSVersion != "legacy-gtfs-version" {
		t.Fatalf("GTFSVersion = %q, want legacy fallback", cfg.Data.GTFSVersion)
	}
}

func TestLoadParsesInternalMTLSServerConfig(t *testing.T) {
	t.Setenv("HTTP_PORT", "8094")
	t.Setenv("INTERNAL_HTTP_TLS_PORT", "9494")
	t.Setenv("HTTP_READ_TIMEOUT", "2s")
	t.Setenv("HTTP_WRITE_TIMEOUT", "3s")
	t.Setenv("HTTP_IDLE_TIMEOUT", "4s")
	t.Setenv("MTLS_MODE", "enforce")
	t.Setenv("MTLS_CA_CERT_PATH", "/run/mtls/ca.crt")
	t.Setenv("MTLS_SERVER_CERT_PATH", "/run/mtls/routing-service/server.crt")
	t.Setenv("MTLS_SERVER_KEY_PATH", "/run/mtls/routing-service/server.key")
	t.Setenv("MTLS_ALLOWED_SPIFFE_IDS", "spiffe://inflap/test/api-gateway")
	t.Setenv("MTLS_ALLOWED_DNS_NAMES", "api-gateway")

	cfg, err := Load(context.Background())
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}

	if cfg.HTTP.Port != 8094 {
		t.Fatalf("HTTP.Port = %d, want 8094", cfg.HTTP.Port)
	}
	if cfg.HTTP.InternalTLSPort != 9494 {
		t.Fatalf("HTTP.InternalTLSPort = %d, want 9494", cfg.HTTP.InternalTLSPort)
	}
	if cfg.HTTP.Address() != ":8094" {
		t.Fatalf("HTTP.Address() = %q, want :8094", cfg.HTTP.Address())
	}
	if cfg.HTTP.InternalTLSAddress() != ":9494" {
		t.Fatalf("HTTP.InternalTLSAddress() = %q, want :9494", cfg.HTTP.InternalTLSAddress())
	}
	if cfg.HTTP.ReadTimeout != 2*time.Second || cfg.HTTP.WriteTimeout != 3*time.Second || cfg.HTTP.IdleTimeout != 4*time.Second {
		t.Fatalf("HTTP timeouts = %s/%s/%s, want 2s/3s/4s", cfg.HTTP.ReadTimeout, cfg.HTTP.WriteTimeout, cfg.HTTP.IdleTimeout)
	}

	serverTransport := cfg.MTLS.ServerConfig()
	if serverTransport.ServerCertPath != "/run/mtls/routing-service/server.crt" ||
		serverTransport.ServerKeyPath != "/run/mtls/routing-service/server.key" ||
		len(serverTransport.AllowedSPIFFEIDs) != 1 ||
		serverTransport.AllowedSPIFFEIDs[0] != "spiffe://inflap/test/api-gateway" ||
		len(serverTransport.AllowedDNSNames) != 1 ||
		serverTransport.AllowedDNSNames[0] != "api-gateway" {
		t.Fatalf("mTLS server config = %+v", serverTransport)
	}
}
