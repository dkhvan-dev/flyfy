...
-- Hainan is a tourist region inside China, while every place remains tied
-- to a concrete city or practical island-area reference.

DROP TABLE IF EXISTS seed_china_hainan_resolved_places;
DROP TABLE IF EXISTS seed_china_hainan_places;
...
CREATE TEMP TABLE seed_china_hainan_places (
...
INSERT INTO seed_china_hainan_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-china-hainan-place:' || seed.slug) AS place_hash,
        md5('id-china-hainan-media:' || seed.slug) AS media_hash
    FROM seed_china_hainan_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_china_hainan_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_china_hainan_resolved_places s
WHERE a.id = s.id;
