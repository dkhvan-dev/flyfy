-- Priority Italy destination places seed.
-- The seed covers Rome and Lazio, Tuscany, Venice and Northern Italy, Campania, Sicily, Puglia, Basilicata, Calabria, and Sardinia.

DROP TABLE IF EXISTS seed_italy_resolved_places;
DROP TABLE IF EXISTS seed_italy_priority_places;

CREATE TEMP TABLE seed_italy_priority_places (
...
INSERT INTO seed_italy_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-italy-place:' || seed.slug) AS place_hash,
        md5('id-italy-media:' || seed.slug) AS media_hash
    FROM seed_italy_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_italy_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_italy_resolved_places seed
WHERE a.id = seed.id;
