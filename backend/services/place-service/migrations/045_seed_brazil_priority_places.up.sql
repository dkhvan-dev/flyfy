-- Priority Brazil destination places seed.
...
DROP TABLE IF EXISTS seed_brazil_resolved_places;
DROP TABLE IF EXISTS seed_brazil_priority_places;

CREATE TEMP TABLE seed_brazil_priority_places (
...
INSERT INTO seed_brazil_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-brazil-place:' || seed.slug) AS place_hash,
        md5('id-brazil-media:' || seed.slug) AS media_hash
    FROM seed_brazil_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_brazil_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_brazil_resolved_places seed
WHERE a.id = seed.id;
