-- Priority Finland destination places seed.
-- Finland is seeded by durable tourist anchors across Helsinki region, coast, Lakeland, Lapland, and Oulu.

DROP TABLE IF EXISTS seed_finland_resolved_places;
DROP TABLE IF EXISTS seed_finland_priority_places;

CREATE TEMP TABLE seed_finland_priority_places (
...
INSERT INTO seed_finland_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-finland-place:' || seed.slug) AS place_hash,
        md5('id-finland-media:' || seed.slug) AS media_hash
    FROM seed_finland_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_finland_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_finland_resolved_places seed
WHERE a.id = seed.id;
