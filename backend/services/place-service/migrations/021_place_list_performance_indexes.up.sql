...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_active_country_category_latest
    ON places (country_code, category, created_at DESC, id)
    WHERE deleted_at IS NULL;
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_active_country_city_price
    ON places (country_code, city_id, price_amount, created_at DESC, id)
    WHERE deleted_at IS NULL
