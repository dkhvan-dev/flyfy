-- Priority Sweden destination places seed.
...
DROP TABLE IF EXISTS seed_sweden_resolved_places;
DROP TABLE IF EXISTS seed_sweden_priority_places;

CREATE TEMP TABLE seed_sweden_priority_places (
...
INSERT INTO seed_sweden_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-sweden-place:' || seed.slug) AS place_hash,
        md5('id-sweden-media:' || seed.slug) AS media_hash
    FROM seed_sweden_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_sweden_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_sweden_resolved_places seed
WHERE a.id = seed.id;
