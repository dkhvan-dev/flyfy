-- Priority Germany destination places seed.
-- The seed keeps major city, nature, culture, shopping, market, and entertainment hubs explicit for localized discovery.

DROP TABLE IF EXISTS seed_germany_resolved_places;
DROP TABLE IF EXISTS seed_germany_priority_places;

CREATE TEMP TABLE seed_germany_priority_places (
...
INSERT INTO seed_germany_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-germany-place:' || seed.slug) AS place_hash,
        md5('id-germany-media:' || seed.slug) AS media_hash
    FROM seed_germany_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_germany_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_germany_resolved_places seed
WHERE a.id = seed.id;
