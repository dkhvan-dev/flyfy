-- First-class place coordinates used by mobile map deep links.
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_places_location_pair'
    ) THEN
        ALTER TABLE places
            ADD CONSTRAINT chk_places_location_pair CHECK (
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_places_latitude'
    ) THEN
        ALTER TABLE places
            ADD CONSTRAINT chk_places_latitude CHECK (
                latitude IS NULL OR (latitude >= -90 AND latitude <= 90)
            );
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_places_longitude'
    ) THEN
        ALTER TABLE places
            ADD CONSTRAINT chk_places_longitude CHECK (
                longitude IS NULL OR (longitude >= -180 AND longitude <= 180)
            );
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_places_location_source_url'
    ) THEN
        ALTER TABLE places
            ADD CONSTRAINT chk_places_location_source_url CHECK (
