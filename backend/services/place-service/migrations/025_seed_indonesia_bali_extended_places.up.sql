-- Extended Bali destination places seed.
-- This migration treats Bali as a tourist destination inside Indonesia while
-- keeping every place attached to a concrete practical city/area.

DROP TABLE IF EXISTS seed_indonesia_bali_extended_resolved;
DROP TABLE IF EXISTS seed_indonesia_bali_extended_places;

CREATE TEMP TABLE seed_indonesia_bali_extended_places (
...
INSERT INTO seed_indonesia_bali_extended_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-indonesia-bali-extended-place:' || seed.slug) AS place_hash,
        md5('id-indonesia-bali-extended-media:' || seed.slug) AS media_hash
    FROM seed_indonesia_bali_extended_places seed
)
SELECT
    (
