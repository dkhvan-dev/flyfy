-- Extended Portugal destination places seed.
-- This migration complements the initial Portugal seed without rewriting already-applied historical migrations.

DROP TABLE IF EXISTS seed_portugal_extended_resolved_places;
DROP TABLE IF EXISTS seed_portugal_extended_places;

CREATE TEMP TABLE seed_portugal_extended_places (
...
INSERT INTO seed_portugal_extended_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-portugal-extended-place:' || seed.slug) AS place_hash,
        md5('id-portugal-extended-media:' || seed.slug) AS media_hash
    FROM seed_portugal_extended_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_portugal_extended_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_portugal_extended_resolved_places seed
WHERE a.id = seed.id;
