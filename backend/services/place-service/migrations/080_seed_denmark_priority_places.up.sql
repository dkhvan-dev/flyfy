-- Priority Denmark destination places seed.
-- Denmark is seeded by major tourist anchors so admin/mobile filters remain city-based and readable.

DROP TABLE IF EXISTS seed_denmark_resolved_places;
DROP TABLE IF EXISTS seed_denmark_priority_places;

CREATE TEMP TABLE seed_denmark_priority_places (
...
INSERT INTO seed_denmark_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-denmark-place:' || seed.slug) AS place_hash,
        md5('id-denmark-media:' || seed.slug) AS media_hash
    FROM seed_denmark_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    CASE
        ...
        WHEN category = 'ENTERTAINMENT' THEN 150::numeric
        ELSE 75::numeric
    END,
    'DKK',
    duration_value,
    ...
    NOW(),
    NOW()
FROM seed_denmark_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_denmark_resolved_places seed
WHERE a.id = seed.id;
