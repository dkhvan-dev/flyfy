-- Priority Philippines places seed.
...
DROP TABLE IF EXISTS seed_philippines_resolved_places;
DROP TABLE IF EXISTS seed_philippines_priority_places;

CREATE TEMP TABLE seed_philippines_priority_places (
...
INSERT INTO seed_philippines_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('ph-place:' || seed.slug) AS place_hash,
        md5('ph-media:' || seed.slug) AS media_hash
    FROM seed_philippines_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_philippines_resolved_places
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_philippines_resolved_places seed
...
WHERE EXISTS (
    SELECT 1
    FROM places a
    WHERE a.id = seed_philippines_resolved_places.id
)
ON CONFLICT (id) DO UPDATE
SET
