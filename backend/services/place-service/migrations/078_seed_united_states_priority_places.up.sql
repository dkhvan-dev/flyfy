-- Priority United States destination places seed.
-- The seed covers major city anchors, national parks, beaches, museums, malls, markets, food halls, and entertainment points.

DROP TABLE IF EXISTS seed_united_states_resolved_places;
DROP TABLE IF EXISTS seed_united_states_priority_places;

CREATE TEMP TABLE seed_united_states_priority_places (
...
INSERT INTO seed_united_states_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-united-states-place:' || seed.slug) AS place_hash,
        md5('id-united-states-media:' || seed.slug) AS media_hash
    FROM seed_united_states_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_united_states_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_united_states_resolved_places seed
WHERE a.id = seed.id;
