-- Priority Portugal destination places seed.
-- The seed keeps tourist hubs explicit for admin filters and localized mobile discovery.

DROP TABLE IF EXISTS seed_portugal_resolved_places;
DROP TABLE IF EXISTS seed_portugal_priority_places;

CREATE TEMP TABLE seed_portugal_priority_places (
...
INSERT INTO seed_portugal_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-portugal-place:' || seed.slug) AS place_hash,
        md5('id-portugal-media:' || seed.slug) AS media_hash
    FROM seed_portugal_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_portugal_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_portugal_resolved_places seed
WHERE a.id = seed.id;
