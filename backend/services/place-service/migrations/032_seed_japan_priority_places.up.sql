-- Priority Japan destination places seed.
-- Japan is kept as one country destination, while every place is tied to
-- a concrete city or tourist hub for reference/admin filters and route search.

DROP TABLE IF EXISTS seed_japan_resolved_places;
DROP TABLE IF EXISTS seed_japan_priority_places;

CREATE TEMP TABLE seed_japan_priority_places (
...
INSERT INTO seed_japan_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-japan-place:' || seed.slug) AS place_hash,
        md5('id-japan-media:' || seed.slug) AS media_hash
    FROM seed_japan_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_japan_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_japan_resolved_places seed
WHERE a.id = seed.id;
