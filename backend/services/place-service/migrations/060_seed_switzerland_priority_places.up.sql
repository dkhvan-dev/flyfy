-- Priority Switzerland destination places seed.
-- The seed covers Zurich, Central Switzerland, Basel, Bernese Oberland, Lake Geneva, Valais, Ticino, and Graubunden.

DROP TABLE IF EXISTS seed_switzerland_resolved_places;
DROP TABLE IF EXISTS seed_switzerland_priority_places;

CREATE TEMP TABLE seed_switzerland_priority_places (
...
INSERT INTO seed_switzerland_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-switzerland-place:' || seed.slug) AS place_hash,
        md5('id-switzerland-media:' || seed.slug) AS media_hash
    FROM seed_switzerland_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_switzerland_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_switzerland_resolved_places seed
WHERE a.id = seed.id;
