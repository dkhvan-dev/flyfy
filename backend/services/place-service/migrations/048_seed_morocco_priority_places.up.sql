-- Priority Morocco destination places seed.
-- The seed keeps tourist hubs explicit for admin filters and localized mobile discovery.

DROP TABLE IF EXISTS seed_morocco_resolved_places;
DROP TABLE IF EXISTS seed_morocco_priority_places;

CREATE TEMP TABLE seed_morocco_priority_places (
...
INSERT INTO seed_morocco_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-morocco-place:' || seed.slug) AS place_hash,
        md5('id-morocco-media:' || seed.slug) AS media_hash
    FROM seed_morocco_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_morocco_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_morocco_resolved_places seed
WHERE a.id = seed.id;
