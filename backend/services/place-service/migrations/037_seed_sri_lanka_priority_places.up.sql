-- Priority Sri Lanka destination places seed.
...
DROP TABLE IF EXISTS seed_sri_lanka_resolved_places;
DROP TABLE IF EXISTS seed_sri_lanka_priority_places;

CREATE TEMP TABLE seed_sri_lanka_priority_places (
...
INSERT INTO seed_sri_lanka_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-sri-lanka-place:' || seed.slug) AS place_hash,
        md5('id-sri-lanka-media:' || seed.slug) AS media_hash
    FROM seed_sri_lanka_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    CASE
        ...
        WHEN category = 'ENTERTAINMENT' THEN 2000::numeric
        ELSE 1000::numeric
    END,
    'LKR',
    duration_value,
    ...
    NOW(),
    NOW()
FROM seed_sri_lanka_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_sri_lanka_resolved_places seed
WHERE a.id = seed.id;
