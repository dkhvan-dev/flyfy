-- Priority Greece destination places seed.
-- The seed covers Athens/Attica, northern and central Greece, islands, and Peloponnese routes.

DROP TABLE IF EXISTS seed_greece_resolved_places;
DROP TABLE IF EXISTS seed_greece_priority_places;

CREATE TEMP TABLE seed_greece_priority_places (
...
INSERT INTO seed_greece_priority_places (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-greece-place:' || seed.slug) AS place_hash,
        md5('id-greece-media:' || seed.slug) AS media_hash
    FROM seed_greece_priority_places seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_greece_resolved_places
...
SET
    ...
    updated_at = NOW()
FROM seed_greece_resolved_places seed
WHERE a.id = seed.id;
