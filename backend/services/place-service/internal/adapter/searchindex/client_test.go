...
func TestClientUpsertSearchDocumentSendsInternalEventRequest(t *testing.T) {
	...
	if gotPayload["sourceService"] != "place-service" ||
		gotPayload["aggregateType"] != "place" ||
		gotPayload["aggregateId"] != "place-1" ||
	...
	if payload["domain"] != "place" ||
		payload["entityId"] != "place-1" ||
		payload["deepLink"] != "/places/place-1" {
...
func TestClientUsesServiceJWTWhenTokenSourceConfigured(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		Domain:   "place",
		EntityID: "place-1",
		Title:    map[string]string{"en": "Place"},
		DeepLink: "/places/place-1",
	})
	if err != nil {
		t.Fatalf("UpsertSearchDocument() error = %v", err)
