...
func main() {
	var (
		...
		minMedia       = flag.Int("commons-min-media-per-place", envIntOr("COMMONS_MIN_MEDIA_PER_PLACE", 0), "minimum total media items per place after importing verified Wikimedia Commons images; 0 disables discovery")
	)
	flag.Parse()
	...
	if err != nil {
		fatalf("load place media rows: %v", err)
	}
	...
	for _, row := range rows {
		...
		if err = updatePlaceMedia(ctx, pool, row.MediaID, fileID); err != nil {
			fatalf("update place media %s: %v", row.MediaID, err)
		}
	...
	if *minMedia > 0 {
		...
		fmt.Println("No place media rows need MinIO backfill.")
	}
}
...
func importCommonsMedia(
	...
	targets, err := loadPlaceTargets(ctx, pool)
	if err != nil {
		return 0, err
	}
...
func loadPlaceTargets(ctx context.Context, pool *pgxpool.Pool) ([]placeTarget, error) {
