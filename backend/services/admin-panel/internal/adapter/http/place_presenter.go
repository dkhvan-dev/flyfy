...
const (
	placeListPageSize                 = 25
	placeExternalMediaPreviewWidth    = 480
	placeExternalMediaMinPreviewWidth = 240
	placeExternalMediaMaxPreviewWidth = 2200
)

var placeCategoryNames = map[string]map[string]string{
...
var placeSourceNames = map[string]map[string]string{
...
var placeCountryValues = []string{"KZ", "KG", "TJ", "MN", "IS", "IE", "NL", "DK", "FI", "EE", "BY", "RS", "GR", "UA", "US", "CA", "SG", "UZ", "RU", "VN", "TH", "PH", "ID", "MV", "SC", "PL", "MX", "BR", "AR", "AB", "CU", "MA", "PT", "IT", "ES", "LU", "DE", "AT", "CH", "SE", "CZ", "FR", "GB", "AU", "NZ", "TZ", "KE", "TR", "AE", "GE", "AZ", "AM", "CN", "KR", "JP", "EG", "MY", "LK", "ME", "IN", "MT", "CY"}

type placeCityReference struct {
	CountryCode string
	CityID      string
}

var placeCityValues = []placeCityReference{
...
var placeCityFilterValues = append([]placeCityReference{
	{CountryCode: "ID", CityID: "bali"},
	{CountryCode: "CN", CityID: "hainan"},
}, placeCityValues...)

var placeCurrencyValues = []string{"KZT", "KGS", "TJS", "MNT", "ISK", "BYN", "RSD", "UAH", "UZS", "USD", "CAD", "SGD", "EUR", "GBP", "CHF", "DKK", "SEK", "CZK", "RUB", "AUD", "NZD", "TZS", "KES", "TRY", "AED", "EGP", "MYR", "LKR", "INR", "VND", "THB", "PHP", "IDR", "MVR", "SCR", "PLN", "MXN", "BRL", "ARS", "CUP", "MAD", "GEL", "AZN", "AMD", "CNY", "KRW", "JPY"}

func placeInputFromItem(item *model.AdminPlace) model.PlaceInput {
	if item == nil {
		return model.PlaceInput{DefaultLocale: "ru", CountryCode: "KZ", Status: "PUBLISHED", Category: "NATURE"}
	}
...
func placeCategoryText(locale string, category string) string {
	raw := strings.TrimSpace(category)
	if raw == "" {
		return "-"
	}
	key := normalizePlaceCode(raw)
	if names, ok := placeCategoryNames[key]; ok {
		return localizedValue(locale, names)
	}
	return humanizeCityID(strings.ToLower(raw))
}

func placeCityText(locale string, countryCode string, cityID string) string {
...
func placeCityNameText(locale string, cityID string) string {
	return displayReferenceCityName(locale, cityID)
}

func placeCountrySearchText(countryCode string) string {
	code := strings.ToUpper(strings.TrimSpace(countryCode))
	if code == "" {
		return ""
	}
...
func placeCitySearchText(countryCode string, cityID string) string {
...
func placeCurrencyText(locale string, currency string) string {
...
func localizedPlaceValue(locale string, values map[string]map[string]string, key string) string {
...
func placeLocaleText(locale string, languageCode string) string {
...
func placeSourceText(locale string, source string) string {
	raw := strings.TrimSpace(source)
	if raw == "" {
		return "-"
	}
	key := normalizePlaceCode(raw)
	if names, ok := placeSourceNames[key]; ok {
		return localizedValue(locale, names)
	}
	return humanizeCityID(strings.ToLower(raw))
}

func placeListMetaText(locale string, item model.AdminPlace) string {
	return fmt.Sprintf("%s: %s · %s: %s",
		translate(locale, "field.defaultLocale"),
		placeLocaleText(locale, item.DefaultLocale),
		translate(locale, "field.source"),
		placeSourceText(locale, item.Source),
	)
}

func normalizePlaceCode(raw string) string {
...
func placeMediaURL(fileID uuid.UUID) template.URL {
	if fileID == uuid.Nil {
		return ""
	}
	return template.URL("/admin/place-media/" + fileID.String())
}

func placeMediaImageURL(item model.AdminPlaceMedia) template.URL {
	if item.FileID != uuid.Nil {
		return placeMediaURL(item.FileID)
	}
	...
	if optimizedURL := placeExternalMediaPreviewURL(parsed, placeExternalMediaPreviewWidth); optimizedURL != "" {
		return template.URL(optimizedURL)
	}
	return template.URL(externalURL)
}

func placeExternalMediaPreviewURL(parsed *url.URL, targetWidth int) string {
	if parsed == nil {
		return ""
	}
	width := clampPlaceExternalMediaPreviewWidth(targetWidth)
	host := strings.ToLower(parsed.Hostname())
	if host == "commons.wikimedia.org" && strings.HasPrefix(parsed.Path, "/wiki/Special:FilePath/") {
		clone := *parsed
		query := clone.Query()
	...
	if host == "upload.wikimedia.org" && strings.HasPrefix(parsed.Path, "/wikipedia/commons/") {
		fileName := wikimediaUploadFileName(parsed.Path)
		if fileName == "" {
			return ""
		}
		return (&url.URL{
			...
			Path:   "/wiki/Special:FilePath/" + fileName,
			RawQuery: url.Values{
...
func clampPlaceExternalMediaPreviewWidth(targetWidth int) int {
	if targetWidth <= 0 {
		return placeExternalMediaPreviewWidth
	}
	if targetWidth < placeExternalMediaMinPreviewWidth {
		return placeExternalMediaMinPreviewWidth
	}
	if targetWidth > placeExternalMediaMaxPreviewWidth {
		return placeExternalMediaMaxPreviewWidth
	}
	return targetWidth
}

func wikimediaUploadFileName(path string) string {
	segments := strings.Split(strings.Trim(path, "/"), "/")
	if len(segments) < 4 || segments[0] != "wikipedia" || segments[1] != "commons" {
		return ""
	}
	if segments[2] == "thumb" {
		if len(segments) < 6 {
		...
		return strings.TrimSpace(segments[5])
	}
	return strings.TrimSpace(segments[len(segments)-1])
}

func placeMediaPosition(position int) int {
	if position < 0 {
		return 1
	}
	return position + 1
...
func placeTagsText(tags []string) string {
	return strings.Join(tags, ", ")
}

func placeCityLinksText(items []model.PlaceCityLink) string {
...
func placeVisitInfoValue(item *model.PlaceVisitInfo, field string) string {
	if item == nil {
		return ""
	}
	switch field {
...
func placeTranslationDescription(input model.PlaceInput, locale string) string {
	if tr, ok := input.Translations[locale]; ok {
...
func placeCategoryOptions(selected string) []PlaceOptionView {
	values := []string{"NATURE", "ARCHITECTURE", "MUSEUM", "BEACH", "PARK", "TEMPLE", "ENTERTAINMENT", "FOOD", "MARKET", "SHOPPING", "OTHER"}
	return placeOptions(values, "place.category.", selected)
}

func placeStatusOptions(selected string) []PlaceOptionView {
	values := []string{"DRAFT", "PUBLISHED"}
	return placeOptions(values, "place.status.", selected)
}

func placeCountryOptions(selected string) []PlaceOptionView {
	selected = strings.ToUpper(strings.TrimSpace(selected))
	if selected == "" {
		selected = "KZ"
	}
	return placeValueOptions(placeCountryValues, selected, true)
}

func placeCountryFilterOptions(selected string) []PlaceOptionView {
	selected = strings.ToUpper(strings.TrimSpace(selected))
	return placeValueOptions(placeCountryValues, selected, true)
}

func placeCityOptions(selected string) []PlaceOptionView {
	return placeCityOptionsFromReferences(placeCityValues, selected)
}

func placeCityFilterOptions(selected string) []PlaceOptionView {
	return placeCityOptionsFromReferences(placeCityFilterValues, selected)
}

func placeCityOptionsFromReferences(values []placeCityReference, selected string) []PlaceOptionView {
...
func placeCurrencyOptions(selected *string) []PlaceOptionView {
	value := ""
	if selected != nil {
		value = strings.ToUpper(strings.TrimSpace(*selected))
	}
	return placeValueOptions(placeCurrencyValues, value, true)
}

func placeValueOptions(values []string, selected string, uppercase bool) []PlaceOptionView {
	seen := make(map[string]bool, len(values)+1)
	out := make([]PlaceOptionView, 0, len(values)+1)
...
func placeLocaleOptions(selected string) []PlaceOptionView {
	values := []string{"ru", "en", "kk"}
	return placeOptions(values, "place.locale.", selected)
}

func placeOptions(values []string, prefix string, selected string) []PlaceOptionView {
	selected = strings.ToUpper(strings.TrimSpace(selected))
	if strings.HasPrefix(prefix, "place.locale.") {
		selected = strings.ToLower(strings.TrimSpace(selected))
		if selected == "" {
			selected = "ru"
		}
	...
	for _, value := range values {
		compare := value
		if strings.HasPrefix(prefix, "place.category.") || strings.HasPrefix(prefix, "place.status.") {
...
func placeListQuery(r valuesReader) PlaceFilterViewData {
	...
	page := parsePlaceListPage(r.Get("page"))
	if countryCode == "" {
		cityID = ""
	}
	values := url.Values{}
	returnValues := url.Values{}
	setPlaceQuery(values, "q", search)
	setPlaceQuery(values, "category", category)
	setPlaceQuery(values, "status", status)
	setPlaceQuery(values, "country", countryCode)
	setPlaceQuery(values, "city", cityID)
	setPlaceQuery(returnValues, "q", search)
	setPlaceQuery(returnValues, "category", category)
	setPlaceQuery(returnValues, "status", status)
	setPlaceQuery(returnValues, "country", countryCode)
	setPlaceQuery(returnValues, "city", cityID)
	if page > 1 {
		returnValues.Set("page", strconv.Itoa(page))
	}
	return PlaceFilterViewData{
		Search:      search,
		Category:    category,
		Status:      status,
...
func placeListURL(query string) string {
	query = strings.TrimSpace(query)
	if query == "" {
		return "/admin/places"
	}
	return "/admin/places?" + query
}
...
func parsePlaceListPage(raw string) int {
...
func placePagination(total int, filters PlaceFilterViewData) PlacePaginationViewData {
	page := filters.Page
	if page < 1 {
		page = 1
	}
	totalPages := 0
	if total > 0 {
		totalPages = (total + placeListPageSize - 1) / placeListPageSize
	}
	from := 0
	to := 0
	if total > 0 {
		from = (page-1)*placeListPageSize + 1
		to = page * placeListPageSize
		if to > total {
			to = total
		}
		if from > total {
	...
	return PlacePaginationViewData{
		Page:          page,
		PageSize:      placeListPageSize,
		Total:         total,
		...
		PreviousQuery: placePageQuery(filters.Query, page-1),
		NextQuery:     placePageQuery(filters.Query, page+1),
		Pages:         placePaginationPages(filters.Query, page, totalPages),
	}
}

func placePaginationPages(baseQuery string, currentPage int, totalPages int) []PlacePaginationPageViewData {
	if totalPages <= 1 {
		return nil
	}
	if currentPage < 1 {
	...
	pageNumbers := placePaginationPageNumbers(currentPage, totalPages)
	pages := make([]PlacePaginationPageViewData, 0, len(pageNumbers))
	for _, page := range pageNumbers {
		if page == 0 {
			pages = append(pages, PlacePaginationPageViewData{IsDots: true})
			continue
		}
		pages = append(pages, PlacePaginationPageViewData{
			Page:      page,
			Query:     placePageQuery(baseQuery, page),
...
func placePaginationPageNumbers(currentPage int, totalPages int) []int {
	if totalPages <= 7 {
...
func placePageQuery(baseQuery string, page int) string {
	if page < 1 {
		page = 1
	}
...
func placePaginationSummary(locale string, pagination PlacePaginationViewData) string {
	if locale == localeRU {
...
func placeCityLinkOptions(selected []model.PlaceCityLink, fallbackCountry string) []PlaceCityLinkOptionView {
	visibleCountry := strings.ToUpper(strings.TrimSpace(fallbackCountry))
	if visibleCountry == "" {
		visibleCountry = "KZ"
	}
	selectedMap := make(map[string]model.PlaceCityLink, len(selected))
	for _, item := range selected {
		...
		selectedMap[country+":"+cityID] = model.PlaceCityLink{CountryCode: country, CityID: cityID}
	}

	out := make([]PlaceCityLinkOptionView, 0, len(placeCityValues)+len(selectedMap))
	seen := make(map[string]bool, len(placeCityValues)+len(selectedMap))
	for _, item := range placeCityValues {
		country := strings.ToUpper(strings.TrimSpace(item.CountryCode))
		cityID := strings.ToLower(strings.TrimSpace(item.CityID))
		if country == "" || cityID == "" {
			continue
		...
		out = append(out, PlaceCityLinkOptionView{
			Value:       value,
			CountryCode: country,
			CityID:      cityID,
			Selected:    selected,
...
func setPlaceQuery(values url.Values, key string, value string) {
...
func parseCityLinks(raw string, fallbackCountry string) []model.PlaceCityLink {
	return parseCityLinkValues([]string{raw}, fallbackCountry)
}

func parseCityLinkValues(values []string, fallbackCountry string) []model.PlaceCityLink {
	...
	for _, item := range items {
		...
		seen[key] = true
		out = append(out, model.PlaceCityLink{
			CountryCode: country,
			CityID:      city,
		})
	}
