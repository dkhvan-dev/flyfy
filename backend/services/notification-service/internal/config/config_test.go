...
func TestLoadMTLSConfigFromEnvironment(t *testing.T) {
	...
	t.Setenv("MTLS_SERVER_CERT_PATH", "/run/mtls/notification-service/server.pem")
	t.Setenv("MTLS_SERVER_KEY_PATH", "/run/mtls/notification-service/server-key.pem")
	...
	if serverTransport.Mode != "enforce" ||
		...
		serverTransport.ServerCertPath != "/run/mtls/notification-service/server.pem" ||
		serverTransport.ServerKeyPath != "/run/mtls/notification-service/server-key.pem" ||
