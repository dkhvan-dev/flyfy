...
func TestClientUpsertSearchDocumentSendsInternalEventRequest(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		...
		EntityID:             "activity-1",
		Locale:               "ru",
		Title:                map[string]string{"ru": "Пикник"},
		DeepLink:             "/activities/activity-1",
		SearchTextNormalized: "пикник",
	})
	if err != nil {
	...
	if gotPayload["sourceService"] != "activity-service" ||
		gotPayload["aggregateType"] != "activity" ||
		gotPayload["aggregateId"] != "activity-1" ||
	...
	if payload["domain"] != "activity" ||
		payload["entityId"] != "activity-1" ||
		payload["deepLink"] != "/activities/activity-1" {
...
func TestClientUsesServiceJWTWhenTokenSourceConfigured(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		Domain:   "activity",
		EntityID: "activity-1",
		Title:    map[string]string{"en": "Activity"},
		DeepLink: "/activities/activity-1",
	})
	if err != nil {
		t.Fatalf("UpsertSearchDocument() error = %v", err)
...
func TestClientUsesConfiguredHTTPClient(t *testing.T) {
	...
	client := New("http://search-service:8101", "internal-token", 2*time.Second, WithHTTPClient(httpClient))
	...
	if !called {
		t.Fatal("configured HTTP client transport was not called")
	}
}
