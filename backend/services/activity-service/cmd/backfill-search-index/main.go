...
func main() {
	...
	batchSize := flag.Int("batch-size", 100, "number of activities to scan per database page")
	limit := flag.Int("limit", 0, "maximum number of activities to scan; 0 means no limit")
	deleteStale := flag.Bool("delete-stale", true, "publish delete events for activities that are no longer indexable")
	flag.Parse()
	...
	stats, err := app.BackfillActivitySearchIndex(ctx, repo, indexer, app.SearchIndexBackfillOptions{
		BatchSize:   *batchSize,
		Limit:       *limit,
		DryRun:      *dryRun,
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to backfill activity search index")
	}
	...
	log.Info().
		...
		Msg("activity search index backfill completed")
}
