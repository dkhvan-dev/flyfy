...
func main() {
	...
	batchSize := flag.Int("batch-size", 50, "number of communities to scan per database page")
	limit := flag.Int("limit", 0, "maximum number of communities to scan; 0 means no limit")
	deleteStale := flag.Bool("delete-stale", true, "publish delete events for communities that are no longer indexable")
	flag.Parse()
	...
	stats, err := app.BackfillCommunitySearchIndex(ctx, repo, indexer, app.SearchIndexBackfillOptions{
		BatchSize:   *batchSize,
		Limit:       *limit,
		DryRun:      *dryRun,
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to backfill community search index")
	}
	...
	log.Info().
		...
		Msg("community search index backfill completed")
}
