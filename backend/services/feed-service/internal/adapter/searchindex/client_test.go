...
func TestClientUpsertSearchDocumentSendsInternalEventRequest(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		...
		Locale:               "ru",
		Title:                map[string]string{"ru": "Гиды"},
		DeepLink:             "/communities/community-1",
		SearchTextNormalized: "гиды",
	})
	if err != nil {
	...
	if gotPayload["sourceService"] != "feed-service" ||
		gotPayload["aggregateType"] != "community" ||
		gotPayload["aggregateId"] != "community-1" ||
	...
	if payload["domain"] != "community" ||
		payload["entityId"] != "community-1" ||
		payload["deepLink"] != "/communities/community-1" {
...
func TestClientUsesServiceJWTWhenTokenSourceConfigured(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		Domain:   "feed",
		EntityID: "post-1",
		Title:    map[string]string{"en": "Post"},
		DeepLink: "/feed/post-1",
	})
	if err != nil {
		t.Fatalf("UpsertSearchDocument() error = %v", err)
	}
