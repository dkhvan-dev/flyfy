...
func TestLocaleFromRequestUsesSupportedAcceptLanguage(t *testing.T) {
	req := httptest.NewRequest(stdhttp.MethodGet, "/v1/posts", nil)
...
func TestWriteUseCaseErrorLocalizesBusinessError(t *testing.T) {
	req := httptest.NewRequest(stdhttp.MethodPost, "/v1/posts", nil)
	req.Header.Set("Accept-Language", "en-US,en;q=0.9")
	rec := httptest.NewRecorder()

	h := &Handler{}
	h.writeUseCaseError(rec, req, app.ErrInvalidPostTitle)

	assertErrorResponse(t, rec, stdhttp.StatusBadRequest, map[string]string{
		"error":   "Check the post title.",
		"message": "Check the post title.",
		"code":    "invalid_post_title",
		"kind":    "business",
	})
}

func TestWriteUseCaseErrorMapsInvalidPostMedia(t *testing.T) {
	req := httptest.NewRequest(stdhttp.MethodPost, "/v1/posts", nil)
	req.Header.Set("Accept-Language", "en-US,en;q=0.9")
	rec := httptest.NewRecorder()

	h := &Handler{}
	h.writeUseCaseError(rec, req, app.ErrInvalidPostMedia)

	assertErrorResponse(t, rec, stdhttp.StatusBadRequest, map[string]string{
		"error":   "Check post media attachments.",
		"message": "Check post media attachments.",
