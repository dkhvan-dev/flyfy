...
func main() {
	...
	batchSize := flag.Int("batch-size", 100, "number of users to scan per database page")
	limit := flag.Int("limit", 0, "maximum number of users to scan; 0 means no limit")
	deleteStale := flag.Bool("delete-stale", true, "publish delete events for users that are no longer indexable")
	flag.Parse()
	...
	stats, err := app.BackfillUserSearchIndex(ctx, repo, indexer, app.SearchIndexBackfillOptions{
		BatchSize:   *batchSize,
		Limit:       *limit,
		DryRun:      *dryRun,
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to backfill user search index")
	}
	...
	log.Info().
		...
		Msg("user search index backfill completed")
}
