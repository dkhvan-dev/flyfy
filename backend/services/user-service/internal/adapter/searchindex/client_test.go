...
func TestClientUpsertSearchDocumentSendsInternalEventRequest(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		...
		EntityID:             "user-1",
		Locale:               "ru",
		Title:                map[string]string{"ru": "Ада"},
		DeepLink:             "/users/user-1",
		SearchTextNormalized: "ада",
	})
	if err != nil {
	...
	if gotPayload["sourceService"] != "user-service" ||
		gotPayload["aggregateType"] != "user" ||
		gotPayload["aggregateId"] != "user-1" ||
	...
	if payload["domain"] != "user" ||
		payload["entityId"] != "user-1" ||
		payload["deepLink"] != "/users/user-1" {
...
func TestClientUsesServiceJWTWhenTokenSourceConfigured(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		Domain:   "user",
		EntityID: "user-1",
		Title:    map[string]string{"en": "User"},
		DeepLink: "/users/user-1",
	})
	if err != nil {
		t.Fatalf("UpsertSearchDocument() error = %v", err)
