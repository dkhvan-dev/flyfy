-- Priority Czechia destination attractions seed.
-- The seed covers Prague, Central Bohemia, South Moravia, South Bohemia, West Bohemia, North Moravia, and key nature regions.

DROP TABLE IF EXISTS seed_czechia_resolved_attractions;
DROP TABLE IF EXISTS seed_czechia_priority_attractions;

CREATE TEMP TABLE seed_czechia_priority_attractions (
...
INSERT INTO seed_czechia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-czechia-attraction:' || seed.slug) AS attraction_hash,
        md5('id-czechia-media:' || seed.slug) AS media_hash
    FROM seed_czechia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_czechia_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Чехия', city_id]::text[],
    NOW(),
    NOW()
FROM seed_czechia_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Czechia', city_id]::text[],
    NOW(),
    NOW()
FROM seed_czechia_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Чехия', city_id]::text[],
    NOW(),
    NOW()
FROM seed_czechia_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_czechia_resolved_attractions seed
WHERE a.id = seed.id;
