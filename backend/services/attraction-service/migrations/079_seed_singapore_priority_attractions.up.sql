-- Priority Singapore destination attractions seed.
-- Singapore is modeled as one city-state destination, with districts and travel themes stored as tags.

DROP TABLE IF EXISTS seed_singapore_resolved_attractions;
DROP TABLE IF EXISTS seed_singapore_priority_attractions;

CREATE TEMP TABLE seed_singapore_priority_attractions (
...
INSERT INTO seed_singapore_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-singapore-attraction:' || seed.slug) AS attraction_hash,
        md5('id-singapore-media:' || seed.slug) AS media_hash
    FROM seed_singapore_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_singapore_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_singapore_resolved_attractions seed
WHERE a.id = seed.id;
