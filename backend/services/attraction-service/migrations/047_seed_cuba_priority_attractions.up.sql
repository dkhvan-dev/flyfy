-- Priority Cuba destination attractions seed.
-- The seed keeps tourist hubs explicit for admin filters and localized mobile discovery.

DROP TABLE IF EXISTS seed_cuba_resolved_attractions;
DROP TABLE IF EXISTS seed_cuba_priority_attractions;

CREATE TEMP TABLE seed_cuba_priority_attractions (
...
INSERT INTO seed_cuba_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-cuba-attraction:' || seed.slug) AS attraction_hash,
        md5('id-cuba-media:' || seed.slug) AS media_hash
    FROM seed_cuba_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_cuba_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Куба', city_id]::text[],
    NOW(),
    NOW()
FROM seed_cuba_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Cuba', city_id]::text[],
    NOW(),
    NOW()
FROM seed_cuba_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Куба', city_id]::text[],
    NOW(),
    NOW()
FROM seed_cuba_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_cuba_resolved_attractions seed
WHERE a.id = seed.id;
