-- Priority Iceland destination attractions seed.
-- The seed covers Reykjavik, the Golden Circle, South Coast, West Iceland, Westfjords, North Iceland and East Iceland.

DROP TABLE IF EXISTS seed_iceland_resolved_attractions;
DROP TABLE IF EXISTS seed_iceland_priority_attractions;

CREATE TEMP TABLE seed_iceland_priority_attractions (
...
INSERT INTO seed_iceland_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-iceland-attraction:' || seed.slug) AS attraction_hash,
        md5('id-iceland-media:' || seed.slug) AS media_hash
    FROM seed_iceland_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_iceland_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_iceland_resolved_attractions seed
WHERE a.id = seed.id;
