-- Priority Seychelles destination attractions seed.
...
DROP TABLE IF EXISTS seed_seychelles_resolved_attractions;
DROP TABLE IF EXISTS seed_seychelles_priority_attractions;

CREATE TEMP TABLE seed_seychelles_priority_attractions (
...
INSERT INTO seed_seychelles_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-seychelles-attraction:' || seed.slug) AS attraction_hash,
        md5('id-seychelles-media:' || seed.slug) AS media_hash
    FROM seed_seychelles_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_seychelles_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_seychelles_resolved_attractions seed
WHERE a.id = seed.id;
