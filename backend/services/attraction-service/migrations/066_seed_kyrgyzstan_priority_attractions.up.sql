-- Priority Kyrgyzstan destination attractions seed.
-- The seed covers Bishkek, Issyk-Kul, Naryn highlands, southern heritage routes, western reservoirs and mountain recreation.

DROP TABLE IF EXISTS seed_kyrgyzstan_resolved_attractions;
DROP TABLE IF EXISTS seed_kyrgyzstan_priority_attractions;

CREATE TEMP TABLE seed_kyrgyzstan_priority_attractions (
...
INSERT INTO seed_kyrgyzstan_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-kyrgyzstan-attraction:' || seed.slug) AS attraction_hash,
        md5('id-kyrgyzstan-media:' || seed.slug) AS media_hash
    FROM seed_kyrgyzstan_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_kyrgyzstan_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Кыргызстан', city_id]::text[],
    NOW(),
    NOW()
FROM seed_kyrgyzstan_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Kyrgyzstan', city_id]::text[],
    NOW(),
    NOW()
FROM seed_kyrgyzstan_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Қырғызстан', city_id]::text[],
    NOW(),
    NOW()
FROM seed_kyrgyzstan_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_kyrgyzstan_resolved_attractions seed
WHERE a.id = seed.id;
