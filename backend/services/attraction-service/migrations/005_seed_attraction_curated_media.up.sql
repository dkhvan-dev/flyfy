...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_attraction_media_external_url'
    ) THEN
        ALTER TABLE attraction_media
            ADD CONSTRAINT chk_attraction_media_external_url
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_attraction_media_source_url'
    ) THEN
        ALTER TABLE attraction_media
            ADD CONSTRAINT chk_attraction_media_source_url
