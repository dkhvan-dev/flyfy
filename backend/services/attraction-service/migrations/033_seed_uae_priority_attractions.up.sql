-- Priority United Arab Emirates destination attractions seed.
-- The UAE is seeded as one country destination, while each attraction is tied
-- to a concrete city or tourist hub for reference/admin filters and route search.

DROP TABLE IF EXISTS seed_uae_resolved_attractions;
DROP TABLE IF EXISTS seed_uae_priority_attractions;

CREATE TEMP TABLE seed_uae_priority_attractions (
...
INSERT INTO seed_uae_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-uae-attraction:' || seed.slug) AS attraction_hash,
        md5('id-uae-media:' || seed.slug) AS media_hash
    FROM seed_uae_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_uae_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_uae_resolved_attractions seed
WHERE a.id = seed.id;
