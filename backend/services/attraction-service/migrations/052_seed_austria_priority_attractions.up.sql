-- Priority Austria destination attractions seed.
-- The seed keeps major city, alpine, lake, shopping, market, museum, and entertainment hubs explicit for localized discovery.

DROP TABLE IF EXISTS seed_austria_resolved_attractions;
DROP TABLE IF EXISTS seed_austria_priority_attractions;

CREATE TEMP TABLE seed_austria_priority_attractions (
...
INSERT INTO seed_austria_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-austria-attraction:' || seed.slug) AS attraction_hash,
        md5('id-austria-media:' || seed.slug) AS media_hash
    FROM seed_austria_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_austria_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Австрия', city_id]::text[],
    NOW(),
    NOW()
FROM seed_austria_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Austria', city_id]::text[],
    NOW(),
    NOW()
FROM seed_austria_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Австрия', city_id]::text[],
    NOW(),
    NOW()
FROM seed_austria_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_austria_resolved_attractions seed
WHERE a.id = seed.id;
