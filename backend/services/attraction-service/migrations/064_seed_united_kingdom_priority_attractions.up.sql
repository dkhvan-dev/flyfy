-- Priority United Kingdom destination attractions seed.
-- The seed covers England, Scotland, Wales, and Northern Ireland tourist anchors with stable year-round POIs.

DROP TABLE IF EXISTS seed_united_kingdom_resolved_attractions;
DROP TABLE IF EXISTS seed_united_kingdom_priority_attractions;

CREATE TEMP TABLE seed_united_kingdom_priority_attractions (
...
INSERT INTO seed_united_kingdom_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-united-kingdom-attraction:' || seed.slug) AS attraction_hash,
        md5('id-united-kingdom-media:' || seed.slug) AS media_hash
    FROM seed_united_kingdom_priority_attractions seed
)
SELECT
    (
...
SET
    ...
    updated_at = NOW()
FROM seed_united_kingdom_resolved_attractions seed
WHERE a.id = seed.id;
