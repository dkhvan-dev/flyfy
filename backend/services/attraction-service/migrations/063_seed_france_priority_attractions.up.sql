-- Priority France destination attractions seed.
...
DROP TABLE IF EXISTS seed_france_resolved_attractions;
DROP TABLE IF EXISTS seed_france_priority_attractions;

CREATE TEMP TABLE seed_france_priority_attractions (
...
INSERT INTO seed_france_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-france-attraction:' || seed.slug) AS attraction_hash,
        md5('id-france-media:' || seed.slug) AS media_hash
    FROM seed_france_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_france_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Франция', city_id]::text[],
    NOW(),
    NOW()
FROM seed_france_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'France', city_id]::text[],
    NOW(),
    NOW()
FROM seed_france_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Франция', city_id]::text[],
    NOW(),
    NOW()
FROM seed_france_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_france_resolved_attractions seed
WHERE a.id = seed.id;
