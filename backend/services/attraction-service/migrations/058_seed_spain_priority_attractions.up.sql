-- Priority Spain destination attractions seed.
...
DROP TABLE IF EXISTS seed_spain_resolved_attractions;
DROP TABLE IF EXISTS seed_spain_priority_attractions;

CREATE TEMP TABLE seed_spain_priority_attractions (
...
INSERT INTO seed_spain_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-spain-attraction:' || seed.slug) AS attraction_hash,
        md5('id-spain-media:' || seed.slug) AS media_hash
    FROM seed_spain_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_spain_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Испания', city_id]::text[],
    NOW(),
    NOW()
FROM seed_spain_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Spain', city_id]::text[],
    NOW(),
    NOW()
FROM seed_spain_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Испания', city_id]::text[],
    NOW(),
    NOW()
FROM seed_spain_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_spain_resolved_attractions seed
WHERE a.id = seed.id;
