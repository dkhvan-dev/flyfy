-- Priority Egypt destination attractions seed.
...
DROP TABLE IF EXISTS seed_egypt_resolved_attractions;
DROP TABLE IF EXISTS seed_egypt_priority_attractions;

CREATE TEMP TABLE seed_egypt_priority_attractions (
...
INSERT INTO seed_egypt_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-egypt-attraction:' || seed.slug) AS attraction_hash,
        md5('id-egypt-media:' || seed.slug) AS media_hash
    FROM seed_egypt_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_egypt_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_egypt_resolved_attractions seed
WHERE a.id = seed.id;
