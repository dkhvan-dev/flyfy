-- Priority Mongolia destination attractions seed.
-- The seed covers Ulaanbaatar, Central Mongolia, the Gobi, Khuvsgul, Altai and eastern steppe routes.

DROP TABLE IF EXISTS seed_mongolia_resolved_attractions;
DROP TABLE IF EXISTS seed_mongolia_priority_attractions;

CREATE TEMP TABLE seed_mongolia_priority_attractions (
...
INSERT INTO seed_mongolia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-mongolia-attraction:' || seed.slug) AS attraction_hash,
        md5('id-mongolia-media:' || seed.slug) AS media_hash
    FROM seed_mongolia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_mongolia_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_mongolia_resolved_attractions seed
WHERE a.id = seed.id;
