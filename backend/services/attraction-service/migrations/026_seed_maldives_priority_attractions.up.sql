-- Priority Maldives destination attractions seed.
-- Maldives is a country destination, while every attraction stays attached to
-- a concrete island, city, or practical atoll reference used by admin filters.

DROP TABLE IF EXISTS seed_maldives_resolved_attractions;
DROP TABLE IF EXISTS seed_maldives_priority_attractions;

CREATE TEMP TABLE seed_maldives_priority_attractions (
...
INSERT INTO seed_maldives_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-maldives-attraction:' || seed.slug) AS attraction_hash,
        md5('id-maldives-media:' || seed.slug) AS media_hash
    FROM seed_maldives_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_maldives_resolved_attractions
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_maldives_resolved_attractions seed
...
WHERE EXISTS (
    SELECT 1
    FROM attractions a
    WHERE a.id = seed_maldives_resolved_attractions.id
)
ON CONFLICT (id) DO UPDATE
SET
