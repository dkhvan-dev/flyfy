-- Priority Armenia destination attractions seed.
-- Armenia is a country destination, while every attraction stays attached to
-- a concrete city, resort, village, or practical regional reference used by admin filters.

DROP TABLE IF EXISTS seed_armenia_resolved_attractions;
DROP TABLE IF EXISTS seed_armenia_priority_attractions;

CREATE TEMP TABLE seed_armenia_priority_attractions (
...
INSERT INTO seed_armenia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
    ...
    ('yerevan-zoo', 'yerevan', 'ENTERTAINMENT', 2, 'HOURS', 4.3, 'Ереванский зоопарк', 'Yerevan Zoo', 'Ереван зообағы', 'Семейная точка рядом с ботаническим садом, животными и образовательными программами.', 'A family attraction near the botanical garden with animals and educational programmes.', 40.19600000, 44.55050000, 'Yerevan Zoo Armenia', 'Yerevan,_Armenia.jpg'),
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-armenia-attraction:' || seed.slug) AS attraction_hash,
        md5('id-armenia-media:' || seed.slug) AS media_hash
    FROM seed_armenia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_armenia_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_armenia_resolved_attractions s
WHERE a.id = s.id;
