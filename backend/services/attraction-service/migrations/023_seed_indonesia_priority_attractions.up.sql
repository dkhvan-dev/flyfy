...
DROP TABLE IF EXISTS seed_indonesia_resolved_attractions;
DROP TABLE IF EXISTS seed_indonesia_priority_attractions;

CREATE TEMP TABLE seed_indonesia_priority_attractions (
...
INSERT INTO seed_indonesia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-bali-attraction:' || seed.slug) AS attraction_hash,
        md5('id-bali-media:' || seed.slug) AS media_hash
    FROM seed_indonesia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_indonesia_resolved_attractions
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_indonesia_resolved_attractions seed
...
WHERE EXISTS (
    SELECT 1
    FROM attractions a
    WHERE a.id = seed_indonesia_resolved_attractions.id
)
ON CONFLICT (id) DO UPDATE
SET
