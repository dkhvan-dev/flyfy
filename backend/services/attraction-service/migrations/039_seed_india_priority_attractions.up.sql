-- Priority India destination attractions seed.
...
DROP TABLE IF EXISTS seed_india_resolved_attractions;
DROP TABLE IF EXISTS seed_india_priority_attractions;

CREATE TEMP TABLE seed_india_priority_attractions (
...
INSERT INTO seed_india_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-india-attraction:' || seed.slug) AS attraction_hash,
        md5('id-india-media:' || seed.slug) AS media_hash
    FROM seed_india_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_india_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_india_resolved_attractions seed
WHERE a.id = seed.id;
