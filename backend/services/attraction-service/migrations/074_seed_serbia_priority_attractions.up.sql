-- Priority Serbia destination attractions seed.
-- The seed covers Belgrade, Vojvodina, south/east Serbia and west/central nature resorts.

DROP TABLE IF EXISTS seed_serbia_resolved_attractions;
DROP TABLE IF EXISTS seed_serbia_priority_attractions;

CREATE TEMP TABLE seed_serbia_priority_attractions (
...
INSERT INTO seed_serbia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-serbia-attraction:' || seed.slug) AS attraction_hash,
        md5('id-serbia-media:' || seed.slug) AS media_hash
    FROM seed_serbia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_serbia_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_serbia_resolved_attractions seed
WHERE a.id = seed.id;
