-- Priority Georgia destination attractions seed.
-- Georgia is a country destination, while every attraction stays attached to
-- a concrete city, town, resort, or practical regional reference used by admin filters.

DROP TABLE IF EXISTS seed_georgia_resolved_attractions;
DROP TABLE IF EXISTS seed_georgia_priority_attractions;

CREATE TEMP TABLE seed_georgia_priority_attractions (
...
INSERT INTO seed_georgia_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-georgia-attraction:' || seed.slug) AS attraction_hash,
        md5('id-georgia-media:' || seed.slug) AS media_hash
    FROM seed_georgia_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_georgia_resolved_attractions
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_georgia_resolved_attractions seed
...
WHERE EXISTS (
    SELECT 1
    FROM attractions a
    WHERE a.id = seed_georgia_resolved_attractions.id
)
ON CONFLICT (id) DO UPDATE
SET
