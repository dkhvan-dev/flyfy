-- Priority Netherlands destination attractions seed.
...
DROP TABLE IF EXISTS seed_netherlands_resolved_attractions;
DROP TABLE IF EXISTS seed_netherlands_priority_attractions;

CREATE TEMP TABLE seed_netherlands_priority_attractions (
...
INSERT INTO seed_netherlands_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-netherlands-attraction:' || seed.slug) AS attraction_hash,
        md5('id-netherlands-media:' || seed.slug) AS media_hash
    FROM seed_netherlands_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_netherlands_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_netherlands_resolved_attractions seed
WHERE a.id = seed.id;
