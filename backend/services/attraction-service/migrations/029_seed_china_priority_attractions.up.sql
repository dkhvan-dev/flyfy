-- Priority China destination attractions seed.
-- China is intentionally kept as one country destination, while every attraction
-- remains tied to a concrete city or tourist hub used by reference and admin filters.

DROP TABLE IF EXISTS seed_china_resolved_attractions;
DROP TABLE IF EXISTS seed_china_priority_attractions;

CREATE TEMP TABLE seed_china_priority_attractions (
...
INSERT INTO seed_china_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-china-attraction:' || seed.slug) AS attraction_hash,
        md5('id-china-media:' || seed.slug) AS media_hash
    FROM seed_china_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_china_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_china_resolved_attractions s
WHERE a.id = s.id;
