-- Priority Tajikistan destination attractions seed.
-- The seed covers Dushanbe day trips, Sughd and Fann Mountains, Pamir/GBAO, and southern Khatlon heritage.

DROP TABLE IF EXISTS seed_tajikistan_resolved_attractions;
DROP TABLE IF EXISTS seed_tajikistan_priority_attractions;

CREATE TEMP TABLE seed_tajikistan_priority_attractions (
...
INSERT INTO seed_tajikistan_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-tajikistan-attraction:' || seed.slug) AS attraction_hash,
        md5('id-tajikistan-media:' || seed.slug) AS media_hash
    FROM seed_tajikistan_priority_attractions seed
)
SELECT
    (
