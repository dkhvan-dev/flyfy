-- Priority Azerbaijan destination attractions seed.
-- The seed covers Baku and Absheron, Gobustan, northern mountains, Sheki-Ganja-Gabala, southern Caspian nature and Nakhchivan.

DROP TABLE IF EXISTS seed_azerbaijan_resolved_attractions;
DROP TABLE IF EXISTS seed_azerbaijan_priority_attractions;

CREATE TEMP TABLE seed_azerbaijan_priority_attractions (
...
INSERT INTO seed_azerbaijan_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-azerbaijan-attraction:' || seed.slug) AS attraction_hash,
        md5('id-azerbaijan-media:' || seed.slug) AS media_hash
    FROM seed_azerbaijan_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_azerbaijan_resolved_attractions
...
SELECT
    ...
    title_ru,
    description_ru,
    ARRAY[title_ru, 'Азербайджан', city_id]::text[],
    NOW(),
    NOW()
FROM seed_azerbaijan_resolved_attractions
UNION ALL
SELECT
    ...
    title_en,
    description_en,
    ARRAY[title_en, 'Azerbaijan', city_id]::text[],
    NOW(),
    NOW()
FROM seed_azerbaijan_resolved_attractions
UNION ALL
SELECT
    ...
    title_kk,
    description_kk,
    ARRAY[title_kk, 'Әзірбайжан', city_id]::text[],
    NOW(),
    NOW()
FROM seed_azerbaijan_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_azerbaijan_resolved_attractions seed
WHERE a.id = seed.id;
