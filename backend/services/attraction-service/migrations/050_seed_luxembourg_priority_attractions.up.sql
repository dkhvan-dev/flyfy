-- Priority Luxembourg destination attractions seed.
-- The seed keeps compact country coverage explicit for admin filters and localized mobile discovery.

DROP TABLE IF EXISTS seed_luxembourg_resolved_attractions;
DROP TABLE IF EXISTS seed_luxembourg_priority_attractions;

CREATE TEMP TABLE seed_luxembourg_priority_attractions (
...
INSERT INTO seed_luxembourg_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-luxembourg-attraction:' || seed.slug) AS attraction_hash,
        md5('id-luxembourg-media:' || seed.slug) AS media_hash
    FROM seed_luxembourg_priority_attractions seed
)
SELECT
    (
...
SET
    ...
    updated_at = NOW()
FROM seed_luxembourg_resolved_attractions seed
WHERE a.id = seed.id;
