-- Priority South Korea destination attractions seed.
-- South Korea is intentionally kept as one country destination, while every
-- attraction remains tied to a concrete city or tourist hub used by reference
-- and admin filters.

DROP TABLE IF EXISTS seed_south_korea_resolved_attractions;
DROP TABLE IF EXISTS seed_south_korea_priority_attractions;

CREATE TEMP TABLE seed_south_korea_priority_attractions (
...
INSERT INTO seed_south_korea_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-south-korea-attraction:' || seed.slug) AS attraction_hash,
        md5('id-south-korea-media:' || seed.slug) AS media_hash
    FROM seed_south_korea_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_south_korea_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_south_korea_resolved_attractions s
WHERE a.id = s.id;
