-- Priority Ukraine destination attractions seed.
-- The seed covers large city anchors, western heritage routes, Black Sea destinations, Carpathian resort points, malls, markets, parks, museums, beaches, and family attractions.

DROP TABLE IF EXISTS seed_ukraine_resolved_attractions;
DROP TABLE IF EXISTS seed_ukraine_priority_attractions;

CREATE TEMP TABLE seed_ukraine_priority_attractions (
...
INSERT INTO seed_ukraine_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-ukraine-attraction:' || seed.slug) AS attraction_hash,
        md5('id-ukraine-media:' || seed.slug) AS media_hash
    FROM seed_ukraine_priority_attractions seed
)
SELECT
    (
...
SET
    ...
    updated_at = NOW()
FROM seed_ukraine_resolved_attractions seed
WHERE a.id = seed.id;
