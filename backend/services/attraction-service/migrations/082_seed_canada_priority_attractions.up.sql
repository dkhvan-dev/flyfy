-- Priority Canada destination attractions seed.
-- The seed covers city anchors, national parks, beaches, museums, malls, markets, food halls, and northern nature points.

DROP TABLE IF EXISTS seed_canada_resolved_attractions;
DROP TABLE IF EXISTS seed_canada_priority_attractions;

CREATE TEMP TABLE seed_canada_priority_attractions (
...
INSERT INTO seed_canada_priority_attractions (
    slug,
    city_id,
    category,
    duration_value,
...
WITH hashed AS (
    SELECT
        seed.*,
        md5('id-canada-attraction:' || seed.slug) AS attraction_hash,
        md5('id-canada-media:' || seed.slug) AS media_hash
    FROM seed_canada_priority_attractions seed
)
SELECT
    (
...
SELECT
    ...
    NOW(),
    NOW()
FROM seed_canada_resolved_attractions
...
SET
    ...
    updated_at = NOW()
FROM seed_canada_resolved_attractions seed
WHERE a.id = seed.id;
