...
func TestArmeniaPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "028_seed_armenia_priority_attractions.up.sql")
	downSQL := readMigration(t, "028_seed_armenia_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_armenia_resolved_attractions AS",
		"'AM'",
		"armenia-seed-v1",
	}
	for _, fragment := range requiredFragments {
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Armenia up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Armenia up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['armenia', city_id") {
		t.Fatalf("Armenia up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "armenia-seed-v1") || !strings.Contains(downSQL, "country_code = 'AM'") {
		t.Fatalf("Armenia down migration must remove only tagged Armenia seed attractions")
	}
}
...
func TestCzechiaPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "062_seed_czechia_priority_attractions.up.sql")
	downSQL := readMigration(t, "062_seed_czechia_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_czechia_resolved_attractions AS",
		"'CZ'",
		"'CZK'",
		"czechia-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Czechia up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Czechia up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['czechia', city_id") {
		t.Fatalf("Czechia up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "czechia-seed-v1") || !strings.Contains(downSQL, "country_code = 'CZ'") {
		t.Fatalf("Czechia down migration must remove only tagged Czechia seed attractions")
	}
}
...
func TestKyrgyzstanPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "066_seed_kyrgyzstan_priority_attractions.up.sql")
	downSQL := readMigration(t, "066_seed_kyrgyzstan_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_kyrgyzstan_resolved_attractions AS",
		"'KG'",
		"'KGS'",
		"kyrgyzstan-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Kyrgyzstan up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Kyrgyzstan up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['kyrgyzstan', city_id") {
		t.Fatalf("Kyrgyzstan up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "kyrgyzstan-seed-v1") || !strings.Contains(downSQL, "country_code = 'KG'") {
		t.Fatalf("Kyrgyzstan down migration must remove only tagged Kyrgyzstan seed attractions")
	}
}
...
func TestBelarusPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "073_seed_belarus_priority_attractions.up.sql")
	downSQL := readMigration(t, "073_seed_belarus_priority_attractions.down.sql")
	...
	for _, fragment := range []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_belarus_resolved_attractions AS",
		"'BY'",
		"'BYN'",
		"belarus-seed-v1",
	} {
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Belarus up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Belarus up migration must include curated attraction %q", title)
		}
	}
	...
	if strings.Contains(upSQL, "highlights") {
		t.Fatalf("Belarus up migration must not write obsolete attraction_translations.highlights column")
	}
	if !strings.Contains(upSQL, "ARRAY['belarus', city_id") {
		t.Fatalf("Belarus up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "belarus-seed-v1") || !strings.Contains(downSQL, "country_code = 'BY'") {
		t.Fatalf("Belarus down migration must remove only tagged Belarus seed attractions")
	}
}
...
func TestCubaPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "047_seed_cuba_priority_attractions.up.sql")
	downSQL := readMigration(t, "047_seed_cuba_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_cuba_resolved_attractions AS",
		"'CU'",
		"'CUP'",
		"cuba-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Cuba up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Cuba up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['cuba', city_id") {
		t.Fatalf("Cuba up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "cuba-seed-v1") || !strings.Contains(downSQL, "country_code = 'CU'") {
		t.Fatalf("Cuba down migration must remove only tagged Cuba seed attractions")
	}
}
...
func TestSpainPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "058_seed_spain_priority_attractions.up.sql")
	downSQL := readMigration(t, "058_seed_spain_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_spain_resolved_attractions AS",
		"'ES'",
		"'EUR'",
		"spain-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Spain up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Spain up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['spain', city_id") {
		t.Fatalf("Spain up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "spain-seed-v1") || !strings.Contains(downSQL, "country_code = 'ES'") {
		t.Fatalf("Spain down migration must remove only tagged Spain seed attractions")
	}
}
...
func TestLuxembourgPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_luxembourg_resolved_attractions AS",
		"'LU'",
		"'EUR'",
		"luxembourg-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Luxembourg up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Luxembourg up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['luxembourg', city_id") {
		t.Fatalf("Luxembourg up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "luxembourg-seed-v1") || !strings.Contains(downSQL, "country_code = 'LU'") {
		t.Fatalf("Luxembourg down migration must remove only tagged Luxembourg seed attractions")
	}
}
...
func TestTanzaniaPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "054_seed_tanzania_priority_attractions.up.sql")
	downSQL := readMigration(t, "054_seed_tanzania_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_tanzania_resolved_attractions AS",
		"'TZ'",
		"'TZS'",
		"tanzania-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Tanzania up migration must seed attraction for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Tanzania up migration must include curated attraction %q", title)
		}
	}
	...
	if !strings.Contains(upSQL, "ARRAY['tanzania', city_id") {
		t.Fatalf("Tanzania up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "tanzania-seed-v1") || !strings.Contains(downSQL, "country_code = 'TZ'") {
		t.Fatalf("Tanzania down migration must remove only tagged Tanzania seed attractions")
	}
}
...
func TestCanadaPriorityAttractionsSeedMigrationCoversTouristBreadth(t *testing.T) {
	upSQL := readMigration(t, "082_seed_canada_priority_attractions.up.sql")
	downSQL := readMigration(t, "082_seed_canada_priority_attractions.down.sql")
	...
	requiredFragments := []string{
		...
		"INSERT INTO attraction_media",
		"INSERT INTO attraction_city_links",
		"CREATE TEMP TABLE seed_canada_resolved_attractions AS",
		"'CA'",
		"'CAD'",
		"canada-seed-v1",
	}
	...
	for _, cityID := range []string{
		...
		if !strings.Contains(upSQL, "'"+cityID+"'") {
			t.Fatalf("Canada up migration must seed attractions for city_id %q", cityID)
		}
	}
	...
	for _, title := range []string{
		...
		if !strings.Contains(upSQL, title) {
			t.Fatalf("Canada up migration must include curated attraction %q", title)
		}
	}
	...
	if strings.Contains(upSQL, "highlights") {
		t.Fatalf("Canada up migration must not write obsolete attraction_translations.highlights column")
	}
	if !strings.Contains(upSQL, "ARRAY['canada', city_id") {
		t.Fatalf("Canada up migration must tag every attraction with the country destination and city")
	}
	if !strings.Contains(downSQL, "canada-seed-v1") || !strings.Contains(downSQL, "country_code = 'CA'") {
		t.Fatalf("Canada down migration must remove only tagged Canada seed attractions")
	}
}
