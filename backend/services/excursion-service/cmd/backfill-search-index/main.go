...
func main() {
	...
	batchSize := flag.Int("batch-size", 100, "number of excursions to scan per database page")
	limit := flag.Int("limit", 0, "maximum number of excursions to scan; 0 means no limit")
	deleteStale := flag.Bool("delete-stale", true, "publish delete events for excursions that are no longer indexable")
	flag.Parse()
	...
	stats, err := app.BackfillExcursionSearchIndex(ctx, repo, indexer, app.SearchIndexBackfillOptions{
		BatchSize:   *batchSize,
		Limit:       *limit,
		DryRun:      *dryRun,
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to backfill excursion search index")
	}
	...
	log.Info().
		...
		Msg("excursion search index backfill completed")
}
