...
func TestNewChatServiceHTTPClientKeepsPlainClientWhenMTLSDisabled(t *testing.T) {
	...
	client, err := newChatServiceHTTPClient(&config.Config{
		ChatService: config.ChatServiceConfig{
			HTTPURL:        "http://chat-service:8088",
...
func TestNewChatServiceHTTPClientFailsFastWhenMTLSEnabledWithoutCA(t *testing.T) {
	...
	_, err := newChatServiceHTTPClient(&config.Config{
		ChatService: config.ChatServiceConfig{
			HTTPURL:        "https://chat-service:9488",
