WITH seed_place_departure_city(place_id, country_code, city_id) AS (
    VALUES
...
direct_landmarks AS (
    ...
    FROM excursions e
    JOIN seed_place_departure_city m ON m.place_id = e.landmark_id
    WHERE e.deleted_at IS NULL
      AND e.departure_city_id IS NULL
),
first_itinerary_places AS (
    SELECT DISTINCT ON (i.excursion_id)
        i.excursion_id AS id,
        m.country_code,
        m.city_id
    FROM excursion_itinerary_items i
    JOIN seed_place_departure_city m ON m.place_id = i.place_id
...
candidates AS (
    ...
    UNION
    SELECT * FROM first_itinerary_places
)
UPDATE excursions e
