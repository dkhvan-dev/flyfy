...
func main() {
	...
	batchSize := flag.Int("batch-size", 100, "number of guide profiles to scan per database page")
	limit := flag.Int("limit", 0, "maximum number of guide profiles to scan; 0 means no limit")
	deleteStale := flag.Bool("delete-stale", true, "publish delete events for guide profiles that are no longer indexable")
	flag.Parse()
	...
	repo := repository.NewPGGuideRepository(pool)
	...
	stats, err := app.BackfillGuideSearchIndex(ctx, repo, userClient, indexer, app.SearchIndexBackfillOptions{
		BatchSize:   *batchSize,
		Limit:       *limit,
		DryRun:      *dryRun,
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to backfill guide search index")
	}
	...
	log.Info().
		...
		Msg("guide search index backfill completed")
}
