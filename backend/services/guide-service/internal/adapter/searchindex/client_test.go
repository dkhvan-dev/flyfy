...
func TestClientUpsertSearchDocumentSendsInternalEventRequest(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		...
		EntityID:             "guide-1",
		Locale:               "ru",
		Title:                map[string]string{"ru": "Гид"},
		DeepLink:             "/guides/user-1",
		SearchTextNormalized: "гид",
	})
	if err != nil {
	...
	if gotPayload["sourceService"] != "guide-service" ||
		gotPayload["aggregateType"] != "guide" ||
		gotPayload["aggregateId"] != "guide-1" ||
	...
	if payload["domain"] != "guide" ||
		payload["entityId"] != "guide-1" ||
		payload["deepLink"] != "/guides/user-1" {
...
func TestClientUsesServiceJWTWhenTokenSourceConfigured(t *testing.T) {
	...
	err := client.UpsertSearchDocument(t.Context(), app.SearchIndexDocument{
		Domain:   "guide",
		EntityID: "guide-1",
		Title:    map[string]string{"en": "Guide"},
		DeepLink: "/guides/guide-1",
	})
	if err != nil {
		t.Fatalf("UpsertSearchDocument() error = %v", err)
	}
