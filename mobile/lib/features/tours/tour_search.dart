List<List<String>> tourSearchNeedleGroups(String value) {
  final tokens = normalizeTourSearchText(value)
      .split(' ')
      .where((token) => token.trim().isNotEmpty)
      .take(6)
      .toList(growable: false);
  return [
    for (final token in tokens) tourSearchNeedleVariants(token),
  ].where((variants) => variants.isNotEmpty).toList(growable: false);
}

List<String> tourSearchNeedleVariants(String token) {
  final variants = <String>[];
  final seen = <String>{};
  void add(String value) {
    final normalized = normalizeTourSearchText(value);
  ...
  if (RegExp(r'[a-z]').hasMatch(token)) {
    add(latinToCyrillicTourSearchText(token));
  }
  if (RegExp(r'[а-яәғқңөұүһі]').hasMatch(token)) {
    add(cyrillicToLatinTourSearchText(token));
  }
  addTourLanguageSearchVariants(add, token);
  return variants;
}

String normalizeTourSearchText(String value) {
  return value
      .trim()
      .toLowerCase()
      .replaceAll('ё', 'е')
...
String latinToCyrillicTourSearchText(String value) {
...
String cyrillicToLatinTourSearchText(String value) {
  final buffer = StringBuffer();
...
void addTourLanguageSearchVariants(
    void Function(String value) add, String token) {
  switch (normalizeTourSearchText(token)) {
    case 'en':
    case 'eng':
    case 'english':
    case 'анг':
