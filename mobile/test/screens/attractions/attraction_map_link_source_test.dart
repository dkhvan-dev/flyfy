...
void main() {
  test(
    'attraction details opens internal app map links instead of OSM source URLs',
    () async {
      ...
      expect(
        source,
        isNot(
          contains('final sourceUrl = attraction.locationSourceUrl.trim();'),
        ),
      );
  ...
  test(
    'attraction source URL migration rewrites OSM links to app map links',
    () async {
      final source = await File(
        '../backend/services/attraction-service/migrations/084_update_attraction_map_links.up.sql',
      ).readAsString();
