...
void main() {
  test('attractions screen renders current-city must visit block before sort',
      () async {
  ...
  test('must visit section is derived from current city attractions safely',
      () async {
    final source = await File(
      'lib/screens/attractions/attractions_screen.dart',
    ).readAsString();
    ...
    final helperStart =
        source.indexOf('List<AttractionVm> _mustVisitAttractions');
    final bodyStart = source.indexOf('Widget _buildBody');
