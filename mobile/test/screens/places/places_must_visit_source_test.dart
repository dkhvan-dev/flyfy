...
void main() {
  test(
    'places screen renders current-city must visit block before sort',
    () async {
  ...
  test(
    'must visit section is derived from current city places safely',
    () async {
      final source = await File(
        'lib/screens/places/places_screen.dart',
      ).readAsString();

      final helperStart = source.indexOf('List<PlaceVm> _mustVisitPlaces');
      final bodyStart = source.indexOf('Widget _buildBody');
