...
void main() {
  test('places filter starts with localized searchable city filter', () async {
    ...
    expect(sheetSource, contains('placeFilterCountrySection'));
    expect(sheetSource, contains('placeFilterCountryAll'));
    expect(sheetSource, contains('placeFilterCountrySearchHint'));
    expect(sheetSource, contains('placeFilterCountryNoResults'));
    ...
    final countrySectionStart = sheetSource.indexOf('AppCountryFilterSection(');
    ...
    expect(screenSource, contains('HomeLocationFilterDefaults.fromPreference'));
  ...
  test('places city filter is hidden until a country is selected', () async {
    final sheetSource = await File(
      'lib/screens/places/places_filter_sheet.dart',
    ).readAsString();
  ...
  test('places filter does not apply duration by default', () async {
    final sheetSource = await File(
      'lib/screens/places/places_filter_sheet.dart',
    ).readAsString();
