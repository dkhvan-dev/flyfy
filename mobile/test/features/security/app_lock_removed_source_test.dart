...
void main() {
  test(
    'app lock pin and biometric implementation is removed from mobile app',
    () async {
      ...
      expect(
        File('lib/core/auth/biometric_auth_service.dart').existsSync(),
        isFalse,
      );

      final pubspec = await File('pubspec.yaml').readAsString();
      final mainSource = await File('lib/main.dart').readAsString();
      final authProviderSource = await File(
        'lib/providers/auth_provider.dart',
      ).readAsString();
      final secureStorageSource = await File(
        'lib/core/storage/secure_storage.dart',
      ).readAsString();
      final loginSource = await File(
        'lib/screens/auth/login_screen.dart',
      ).readAsString();
      final profileSecuritySource = await File(
      ...
      expect(
        ...
        isNot(contains('profileSecurityBiometric')),
      );
      expect(
        profileSecuritySource,
      ...
      expect(
        editProfileSource,
        isNot(contains('profileSettingsSecuritySection')),
      );
      expect(
        editProfileSource,
        isNot(contains('profileSettingsSecurityPinTitle')),
      );
