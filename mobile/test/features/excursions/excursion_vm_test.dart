...
void main() {
  test('parses public excursion list fields from excursion-service response',
      () {
  ...
  test('parses public excursion detail fields from excursion-service response',
      () {
    final excursion = ExcursionVm.fromJson(const {
      'id': 'excursion-1',
  ...
  test('parses localized excursion copy from product and offer responses', () {
    ...
    expect(excursion.offers.first.translations['kk']?.title,
        'Менің Шарын бағытым');
    expect(
  ...
  test('localizes landmark based excursion title from attraction translations',
      () {
    const excursion = ExcursionVm(
      id: 'excursion-product-1',
